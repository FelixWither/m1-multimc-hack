...
def lwjglver():
    ...
    for entries in hmcl_config['libraries']:
        for keys in entries:
            if str(entries[keys]).find('lwjgl') != -1:
                if not isinstance((entries[keys]), dict):
                    if str(entries[keys]).find('lwjgl:2.') != -1:
                        GLlist.append(str(entries[keys]))
                    elif str(entries[keys]).find('lwjgl:3.') != -1:
                        GLlist.append(str(entries[keys]))
    lwjgl_ver = GLlist[0]
    if lwjgl_ver == 'org.lwjgl.lwjgl:lwjgl:2.9.4-nightly-20150209':
